...
THS_BI.CalMainBusiness = function (stockCode, stockName, source) {
    ...
    for (var i = 0; i < business.length; i++) {
        THS_BI.MemData.Enqueue("主营业务", business[i], { StockCode: stockCode, StockName: stockName });
    }
}
...
THS_BI.CalFunds = function (stockCode, stockName, source) {
    ...
    ///最近一日大单占比-16%至-10%
    if (-16 <= e1 &&  e1 <=-10) {
        THS_BI.MemData.Enqueue("最近一日大单占比-16%至-10%", "负值", { StockCode: stockCode, StockName: stockName });
    }
    ...
    ///最近一日中单占比-7%至-2%
    if (-7 <= g1 && g1 <= 3) {
        THS_BI.MemData.Enqueue("最近一日中单占比-7%至3%", "负值", { StockCode: stockCode, StockName: stockName });
    }

    ///最近一日大中单占比-16%至-8%
    if ((-16 <= e1 && e1 <= -10) && (-7 <= g1 && g1 <= 3)) {
        THS_BI.MemData.Enqueue("最近一日大中单占比均-16%至-8%", "负值", { StockCode: stockCode, StockName: stockName });
    }
...
THS_BI.RecommandStock = function () {
    ...
    ///新推荐 高概率上涨
    var stockArrayV2 = THS_BI.MemData["目标股票"]["涨幅-0,5%至0,5%"];
    var w1 = { name: "最近一日大单占比-16%至-10%", data: THS_BI.MemData["最近一日大单占比-16%至-10%"] };
    var w2 = { name: "最近一日中单占比-16%至-10%", data: THS_BI.MemData["最近一日中单占比-16%至-10%"] };
    var w3 = { name: "最近一日大中单占比均-16%至-8%", data: THS_BI.MemData["最近一日大中单占比均-16%至-8%"] };
    var w = [w1, w2, w3];
    for (var i = 0; i < stockArrayV2.length; i++) {
        var stock2 = stockArrayV2[i];
        for (var j = 0; j < w.length; j++) {
            var ww = w[j].data;
            var cname2 = w[j].name;
            for (var key2 in ww) {
                var exq2 = ww[key2];
                for (var k = 0; k < exq2.length; k++) {
                    var test2 = exq2[k];
                    if (stock2.StockCode === test2.StockCode) {
                        stock2.Hit += 1;
                        stock2.Source.push(cname2 + ">" + key2);
                    }
                }
            }
        }
    ...
    for (var i = 0; i < stockArrayV2.length; i++) {
        var stock2 = stockArrayV2[i];
        if (undefined === hitDict2[stock2.Source.length]) {
            hitDict2[stock2.Source.length] = [];
        }
        hitDict2[stock2.Source.length].push(stock2);
    }

    THS_BI.MemData.AddObject("前日大抛售", "高概率数据", hitDict2);
}
...
THS_BI.CalTargetStock = function (stockCode, stockName, source) {
    ...
    if (2<=((item1.Closing - item2.Closing) / item2.Closing*100)) {///股票涨幅
    ...
    if (-0.5 <= ((item1.Closing - item2.Closing) / item2.Closing * 100) &&  ((item1.Closing - item2.Closing) / item2.Closing * 100)<=0.5 ) {///股票涨幅
        THS_BI.MemData.Enqueue("目标股票", "涨幅-0,5%至0,5%", { StockCode: stockCode, StockName: stockName, increase: ((item1.Closing - item2.Closing) / item2.Closing * 100), Hit: 0, Source: [] });
    }
}
