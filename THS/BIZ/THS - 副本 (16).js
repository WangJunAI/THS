...
var THS = {
    ...
    TraversePager_MutilDTo2DV2: function () {
        ...
        sourceArray.push({ CollectionName: "DataKLine", Filter: {}, DB: sourceDB, Pager: { Index: 0, Size: 100 } });
        //sourceArray.push({ CollectionName: "DataGGLHB", Filter: {}, DB: sourceDB, Pager: { Index: 0, Size: 10 } });
        //sourceArray.push({ CollectionName: "DataStock", Filter: {}, DB: sourceDB, Pager: { Index: 0, Size: 10 } });


        var callback = function (dbItem, pagerInfo, isLastItem) {
            console.log("TraversePager_MutilDTo2DV2 当前遍历位置 " + pagerInfo.CollectionName + " " + pagerInfo.CurrentIndex + " " + pagerInfo.PageSize);

            var arr = THSPageKLine.ConvertTo2D(dbItem);///二维化
            while (0 < arr.length) {
                var arrItem = arr.shift();
                targetDB.Save(targetCollectionMap[pagerInfo.CollectionName], arrItem, function (err, res, remaining) {
                    if (0 === remaining) {
    ...
    TraversePager_Data: function () {
        ...
        sourceArray.push({ CollectionName: "DataStock", Filter: { $and: [{ "ContentType": { $eq: "首页概览" } }, { "Company.Value.8": { $gt: 50 } }, { "Company.Value.8": { $gt: 50 } }, { "Company.Value.10": { $gt: 1 } }, { "Company.Value.11": { $gt: 1 } }, { "Company.Value.12": { $gt: 1 } }] }, DB: sourceDB, Pager: { Index: 0, Size: 100 } });
        ...
        var callback = function (dbItem, pagerInfo, isLastItem) {
            ...
            if ("DataKLine" === pagerInfo.CollectionName && "日线数据" === pagerInfo.Filter.ContentType) { 
                ...
                THSDataAnalyseV2.LoadDataSource("个股龙虎榜字典", dbItem, { AsDict: true, Keys: "StockCode" });
            }
            ...
            else if ("DataStock" === pagerInfo.CollectionName && "首页概览" === pagerInfo.Filter.$and[0].ContentType.$eq) {
                THSDataAnalyseV2.LoadDataSource("优质股字典", dbItem, { AsDict: true, Keys: "StockCode" });
            }
    ...
    TraversePager_PageSINA: function () {
        ...
        var targetCollectionMap = { "DaDan": "DataDaDan", "DataDaDan": "DataDaDan2D"};
        var sourceArray = [];

        sourceArray.push({ CollectionName: "DaDan", Filter: {}, DB: sourceDB, Pager: { Index: 0, Size: 100 } });
        sourceArray.push({ CollectionName: "DataDaDan", Filter: {}, DB: sourceDB, Pager: { Index: 0, Size: 100 } });
